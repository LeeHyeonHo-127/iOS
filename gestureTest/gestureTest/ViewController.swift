...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    self.view.addGestureRecognizer(self.nextGesture)
      self.view.addGestureRecognizer(self.beforeGesture)
    self.nextGesture.addTarget(self, action: #selector(handleGesture(_:)))
      self.beforeGesture.addTarget(self, action: #selector(handleBeforeGesture(_:)))
  ...
  @objc private func handleGesture(_ gesture: UIPanGestureRecognizer) {
    ...
    @objc private func handleBeforeGesture(_ gesture: UIPanGestureRecognizer) {
      switch gesture.state {
      case .began:
        self.view.isUserInteractionEnabled = false
        self.animator = self.getNextAnimator()
  ...
  private func getNextAnimator() -> UIViewPropertyAnimator? {
    ...
    private func getBeforeAnimator() -> UIViewPropertyAnimator? {
      self.animator?.stopAnimation(false) // true인 경우 completionHandler호출없이 inactive상태, false인 경우 애니메이션이 멈춘 Stopped상태 (finishAnimation과 같이 사용)
      self.animator?.finishAnimation(at: .end) // completion handler 호출, inactive 상태로 전환
